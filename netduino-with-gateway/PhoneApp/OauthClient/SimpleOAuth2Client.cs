...
namespace WindowsAzure.Acs.Oauth2.Client.WinRT
{
    ...
    public class SimpleOAuth2Client
    {
        ...
        public async Task AuthorizeAsync(string refreshToken = null)
        {
            if (Mode == ClientMode.ThreeLegged || refreshToken != null)
            {
                await AuthorizeThreeLegged(refreshToken);
            }
            else
            {
                await AuthorizeTwoLegged();
            }

        }

        private async Task AuthorizeThreeLegged(string refreshToken)
        {
            ...
            var f = new FormUrlEncodedContent(authorizeRequest.Parameters);
            //var f2 = serializer.GetFormEncodedQueryFormat(authorizeRequest);

            var response = await req.PostAsync(authorizeRequest.BaseUri, f);
        ...
        private async Task AuthorizeTwoLegged()
        {
            var authorizeRequest = BuildAuthorizationCodeRequest();
            ...
            var message = deserializedMessage as EndUserAuthorizationResponse;
            if (message != null)
            {
                await AuthorizeThreeLegged(HttpUtility.UrlDecode(message.Code));
            }
        ...
        /// </summary>
        /// <param name="client">The client.</param>
        public async Task AppendAccessTokenToAsync(HttpWebRequest request)
        {
            if (CurrentAccessToken == null)
            {
                throw new ArgumentNullException("CurrentAccessToken is null. A call to Authorize() using an authorization or refresh token should be made first.", "CurrentAccessToken");
            ...
            request.Headers["Authorization"] = new AuthenticationHeaderValue("Bearer", Convert.ToBase64String(Encoding.UTF8.GetBytes(CurrentAccessToken.AccessToken))).ToString();
        }
